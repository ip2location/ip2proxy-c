...
//Static variables
static enum IP2Proxy_mem_type DB_access_type = IP2PROXY_FILE_IO;
...
//Static functions
static int32_t IP2Proxy_DB_Load_to_mem(FILE *filehandle, void *cache_shm_ptr, int64_t size);
...
{
    struct stat statbuf;
    DB_access_type = IP2PROXY_CACHE_MEMORY;
    ...
    if( IP2Proxy_DB_Load_to_mem(filehandle, cache_shm_ptr, statbuf.st_size) == -1 )
    {
        DB_access_type = IP2PROXY_FILE_IO;
        free(cache_shm_ptr);
        return -1;
...
int32_t IP2Proxy_DB_set_shared_memory(FILE *filehandle)
{
    ...
    DB_access_type = IP2PROXY_SHARED_MEMORY;
...
int32_t IP2Proxy_DB_Load_to_mem(FILE *filehandle, void *memory, int64_t size)
{
...
int32_t IP2Proxy_DB_close(FILE *filehandle)
{
    ...
    if ( filehandle != NULL )
        fclose(filehandle);
    if ( DB_access_type == IP2PROXY_CACHE_MEMORY )
    {
        if( cache_shm_ptr != NULL )
            free(cache_shm_ptr);
    }
    else if ( DB_access_type == IP2PROXY_SHARED_MEMORY )
    {
        if( cache_shm_ptr != NULL )
        {
#ifndef	WIN32
...
struct in6_addr_local IP2Proxy_readIPv6Address(FILE *handle, uint32_t position)
{
    ...
    {
        addr6.u.addr8[i] = IP2Proxy_read8(handle, position + j);
    }
    return addr6;
}
